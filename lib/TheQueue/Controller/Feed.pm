...
sub feed {
  ...
  my $search = {ts => {'$gt' => DateTime->now->subtract(days => 120)->stringify}};

  $self->feeds->search($search)->all(sub {
    my ($feeds, $err, $feed) = @_;
        ...
        openapi => {
          feed => [
            reverse map {
              ...
              my $ret  = {};
              $ret->{msg}        = $item->msg;
              $ret->{user}       = {username => $item->user->username, id => $item->user->id} if $item->user;
              $ret->{submission} = {
                ...
                submitter   => {username => $item->submission->user->username, id => $item->submission->user->id},
                interested  => [map { {username => $_->username, id => $_->id} } @{$item->submission->interested}]
                  }
                  if $item->submission;
              $ret;
            } @{$feed}
