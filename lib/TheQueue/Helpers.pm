...
sub register {
  ...
  $app->helper(
    gen_pwhash => sub {
      ...
      my $password_hash = en_base64(bcrypt_hash({key_nul => 1, cost => $cost, salt => $salt}, $password));
  ...
  $app->helper(
    auth => sub {
      my $self = shift;
      return 1 if $self->session and $self->session('logged_in') and $self->session('logged_in') == 1;
      if ($self->req->headers->authorization) {
        ...
        if ($user) {
          ...
          my $cost = $user->cost;
          if ($salt and $cost) {
            my $password_hash = $self->gen_pwhash({salt => $salt, cost => $cost, password => $password})->{hash};
